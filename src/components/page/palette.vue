...
export default {
  ...
  data() {
    return {
      paletteStyleCopy: {
        title: ``,
        width: 1280,//document.body.clientWidth - 20
        height: 720,//document.body.clientHeight - 75
        backgroundColor: 'white',
        borderStyle: 'solid',
        borderColor: '#fed640',
        borderWidth: 0,
        defaultColor: 'red', // 全局默认颜色
        defaultLineWidth: 8 // 全局默认线条宽度
      },
      showbarOptionCopy: {
        clearBtn: true,
        saveBtn: {
  ...
  mounted() {
    ...
    this.init();
    console.log(this.paletteStyleCopy.width,'width')
    console.log(this.paletteStyleCopy.height,'height')
    console.log(this.hua,'this.hua')
  ...
  methods: {
    ...
    //   console.log(e,'我是组件过程中的移动')
    // },
    
    // 手绘
    startPoint(e) {
      // this.$res.palette.style.zIndex=1
    ...
    movePoint(e) {
      ...
      // console.log(this.move.x,'中间x111')
      // console.log(this.move.y,'中间y112，若是最后一个即结束')  //也是橡皮擦经过的位置
      let single={
          x:this.move.x,
          y:this.move.y
        }
        this.xian.push(single)
    },
    ...
    endPoint() {
      ...
      if (this.touchType == 'Random') {
        if (this.startNew) {
          ...
          this.ctx.fill();
          this.xiantiao.push(this.xian)
          this.xian=[]
          console.log(this.xiantiao,'鼠标抬起的坐标xiantiao最终数组')
        }
      }
    ...
    // 清除画布
    clearPalette() {
      // console.log(this.startNew,'this.startNew')
      // console.log(this.startOld,'this.startOld')
      // console.log(this.history,'this.history')
      // console.log(this.currectHistory,'this.currectHistory')
      // return
      this.clearTimes++;
      this.ctx.clearRect(
        0,
